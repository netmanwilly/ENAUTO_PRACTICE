...
def getDeviceFamilyIdentifiers():
    API_URI = '/dna/intent/api/v1/image/importation/device-family-identifiers'
...
def getSoftwareImageDetails():
    API_URI = '/dna/intent/api/v1/image/importation'
    BASE_URI = 'https://sandboxdnac2.cisco.com'
    HEADERS = {
        'X-Auth-Token': getToken(),
        'Content-Type': 'application/json',
    ...
    if RESPONSE.status_code != 200:
        raise ValueError(RESPONSE.status_code)
    return RESPONSE.json()
print(json.dumps(getSoftwareImageDetails(), indent=2))